...
document.getElementById('formMovimiento').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if(response.ok) {
            ...
            alert("Operacion registrada exitosamente en el sistema.");
        } else {
            alert("Error interno al procesar la transaccion.");
        }
    } catch (error) {
        console.error(error);
    } finally {
...
async function enviarAlertaWhatsApp() {
    ...
    try {
        ...
        if(data.mensaje_generado.includes("Todo en orden")) {
            alert("El inventario se encuentra en niveles optimos.");
            toggleLoader(false);
            return;
        }
        
        const numeroDueno = "51982887891";
        const url = `https://wa.me/${numeroDueno}?text=${encodeURIComponent(data.mensaje_generado)}`;
        window.open(url, '_blank');
        
    } catch (error) {
        alert("Fallo de conexion con el servidor de alertas.");
    } finally {
        toggleLoader(false);
    }
}
